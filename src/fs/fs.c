...
/* 格式化整个分区,也就是初始化分区的元信息, 创建文件系统 */
static void partition_format(struct partition *part)
{
    /* 为方便实现,一个块大小是一扇区 */
    uint32_t boot_sector_sects = 1;     // 一个启动块
    uint32_t super_block_sects = 1;     // 一个超级块
    ...
    sb.block_bitmap_sects = block_bitmap_sects; // 数据块位图的扇区数量

    sb.inode_bitmap_lba = sb.block_bitmap_lba + sb.block_bitmap_sects;  // inode位图扇区的LBA
    sb.inode_bitmap_sects = inode_bitmap_sects;   // inode位图的扇区数量       

    sb.inode_table_lba = sb.inode_bitmap_lba + sb.inode_bitmap_sects;   // inode表扇区的LBA
    sb.inode_table_sects = inode_table_sects;     // inode表占用的扇区数量

    sb.data_start_lba = sb.inode_table_lba + sb.inode_table_sects; // 数据区的起始就是inode数组的结束
    sb.root_inode_no = 0;   // 根目录的inode号
...
static bool mount_partition(struct list_elem* pelem, int arg) {
    ...
    if (!strcmp(part->name, part_name)) {   // 默认为sdb1
        cur_part = part;
...
void filesys_init()
{
    ...
    while (channel_no < channel_cnt) { // 遍历两个通道
        ...
        while (dev_no < 2) {    // 遍历通道下1主1从两个硬盘
            ...
            struct partition *part = hd->prime_parts;
...
int32_t sys_read(int32_t fd, void *buf, uint32_t count)
{
    ...
    else if (fd == stdin_no)
    {
        ...
        {
            *buffer = ioqueue_getchar(&kbd_buf);
            bytes_read++;
            buffer++;
        }
