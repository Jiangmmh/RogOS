...
void inode_sync(struct partition* part, struct inode* inode, void* io_buf) {
    ...
    // inode中的成员 inode_tag、i_open_cnts和write_deny在磁盘中是不起作用的
    // 它们只在内存中记录链表位置和被多少进程共享
    struct inode pure_inode;
    memcpy(&pure_inode, inode, sizeof(struct inode));   // 新建一个inode副本，避免修改原inode

    // 以下 inode 的三个成员只存在于内存中
    // 现在将 inode 同步到硬盘, 清掉这三项，防止下次将inode加载到内存时出现问题
...
struct inode* inode_open(struct partition* part, uint32_t inode_no) {
    ...
    // 为了通过 sys_malloc 创建的新 inode 被所有任务共享
