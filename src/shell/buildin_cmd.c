...
void buildin_ls(uint32_t argc, char **argv)
{
    ...
    else    // 解析参数中的路径，将其转换为绝对路径
    {
        make_clear_abs_path(pathname, final_path);
        pathname = final_path;
    }

    if (stat(pathname, &file_stat) == -1)   // 获取路径中目录或文件状态
    {
        printf("ls: cannot access %s: No such file or directory\n", pathname);
        return;
    }

    if (file_stat.st_filetype == FT_DIRECTORY)  // ls的参数为目录，打印其中所有文件和目录的信息
    {
        ...
        if (long_info)  // 是否打印更多信息，通过-l参数来确定
        {
            char ftype;
            printf("total: %d\n", file_stat.st_size);
